...
import {
   ...
   addUpdateCategory,
   TrashCategory,
} from "../../api/promotion-category/index.js";
...
const Page = () => {
         ...
         if (result?.status == true) {
            ...
            result.categories.map((item, index) =>
               categoriesArray.push({
                  ...
                  img: ProfileImageComposer(item?.promotion_cat_image_icon),
                  status: item?.status,
                  action: "Action",
               })
            );
   ...
   const categoryAction = async () => {
      ...
      try {
         ...
         const jsonString = result;
         console.log("jsonStringjsonString", jsonString);
         const jsonObject = JSON.parse(jsonString);
         if (jsonObject?.status == true) {
            getAllCategories();
            ...
            <>
               <Card className="mt-5 mx-5">
                  <div className="row my-4">
                     <div className="col-6">
                        <h4 className="ms-3 ">Promotion category list</h4>
                     </div>
